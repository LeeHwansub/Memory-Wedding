...
### 디자인 페이지 UX
- **데스크톱**: 좌측 설정 + 우측 sticky 폰 미리보기
- **모바일**: `설정` / `미리보기` 탭 전환 + 하단 고정 저장 바
- 메인 사진 **클릭으로 초점** 지정 (십자 마커)
- 갤러리 **다중 파일** 업로드, 썸네일 그리드에서 순서·삭제
- 썸네일 **드래그 앤 드롭** 순서 변경
- 설정 변경 시 **자동 저장**(약 1.1초 debounce) + 미저장 배지
- 미저장 변경 배지 + 변경 있을 때만 수동 저장 활성화
